...
Because reversion regenerates the deterministic base, a regrown seam or floor is exactly the one the seed always had there — no new placement logic. Edits inside an active deed (a tended cottage's builds, a working mine's excavation) never regrow by this path; only edits outside any active deed do.

**Trees fell whole and regrow from saplings (James's call) — the one exception to instant reversion.** Two linked fixes:
- **Whole-tree felling, no floating canopy.** Chopping a trunk fells the **whole tree** — the connected `LOG`/`LEAVES` above come down at once (logs drop as timber, the canopy clears), not left hanging in mid-air (today, breaking one trunk block strands the rest). A flood-fill of connected wood from the cut, on break.
- **Gradual regrowth — a sapling that matures.** A felled tree does **not** pop back full-grown: after its regrowth delay a new **sapling** sprouts on the spot and **grows in stages** to a full tree over further game-days; a sapling or young tree yields little timber, a mature one a full load. So clear-felling leaves a wood of saplings, and there is a real **incentive to let a wood recover before cutting again** — sustainable forestry, not strip-felling. (Needs a new sapling block + a slow growth tick; the felled base is marked in the ledger so the sapling sprouts there. Plants, ore and peat still use the instant forget-the-edit reversion — only trees grow gradually.)
...
**Lapse:** when `now > paidUntilDay + grace`, the claim **lapses** (`lapsedDay` set). Its protection drops, and its build edits enter **gradual reclamation**: from `lapsedDay`, the build edits within it expire a few at a time **slowly, over many days** (a visible ruining — moss, gaps, then moor), never a sudden vanish; the lapse grace and this reclamation are deliberately **generous** (§14b), so a week away never costs a tended home. **Decay is also world-mode-aware (James's call): the bairns world decays slower still** — about twice the grace and reclamation time of adult worlds — because children play in bursts and grow attached to what they build (`game.bairnLocked()` scales the decay timing). **Reclaimable:** return and pay before reclamation completes and the claim is restored (surviving builds stay; fully-reverted blocks are gone). A fully-reclaimed claim is removed and the land is open to claim again.
...
- **Slice 1 — the ledger + flora/ore regrowth:** `editledger.js` (pure + headless) + edit metadata + client expiry generalising the beach-revert; surface **peat** banks (cut at grade + regrow + lowest price); **trees fell whole and regrow gradually from saplings**. Proves "the moor heals" single-player.
...
- **Slice 3 — land claims (homesteads):** apply deeds to surface land — build-edit protection + lapse → gradual reclamation + warnings + **world-mode decay scaling** (bairns world slower, §14n).
...
o. **Trees: whole-fell + gradual sapling regrowth (James's call, 2026-06-20):** chopping a trunk fells the whole connected tree (no floating canopy); a felled tree regrows as a **sapling that matures in stages** over game-days, young trees yielding little — an incentive to let woods recover before re-cutting. Needs a new sapling block + a growth tick + a whole-tree fell flood-fill; part of Slice 1's tree handling (trees are the exception to the instant forget-the-edit reversion).
n. **Decay slower in the bairns world (James's call, 2026-06-20):** structure/claim decay — the lapse grace, the reclamation duration, and the unclaimed-build lifespan — is multiplied by a **world-mode factor**, roughly **2× slower in the bairns room than in adult worlds**, so children's builds are extra forgiving. A single knob: `decayScale = game.bairnLocked() ? 2 : 1`. Healing (regrowth) stays uniform across worlds unless tuned otherwise. Applies in Slices 3–4 (decay), not Slice 1 (healing).
