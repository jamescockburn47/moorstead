...
assert.equal(FUTURE_BLOCKS.length, 9);
...
assert.deepEqual(FUTURE_BLOCKS.slice(6).map(row => [row.id, row.tile, row.name]),
  [[206, 126, 'Vehicle control'], [207, 127, 'Sandbags'], [208, 128, 'Bunker armour']]);
...
assert.equal(hashes.size, 9, 'vehicle and battlefield materials add distinct art to the original six blocks');
...
// reserved cells and that all four sides and corners replicate payload edges.
...
assert.equal(patches.length, 9, 'repeat painting reuses the same atlas without work or allocation');
assert.equal(uploads, 1, 'one GPU upload request for nine static tiles');
...
assert.deepEqual(Object.keys(BLOCKS), [...originalIds, '200', '201', '202', '203', '204', '205', '206', '207', '208'], 'registration adds only nine private IDs');
assert.ok(readFileSync(new URL('../src/freeplay/future-blocks.js', import.meta.url), 'utf8').split('\n').length <= 300);
console.log('PASS free-play future blocks: isolated/idempotent registration, append-only vehicle and battlefield IDs, nine distinct opaque tiles, gutters, one atlas upload and Uint8 chunk persistence.');
