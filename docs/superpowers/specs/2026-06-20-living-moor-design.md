...
**Quarrying vs mining — where building stone comes from (James's flag).** So the 1-block rule never blocks basic building: **building stone is quarried at the surface, free and unlicensed.** The moors expose rock at **crags, scars and dale-sides** (`rocky` ground, where stone *is* the surface) and **beck beds** yield cobble and gravel — all taken within the 1-block-of-local-grade rule (the rock is the grade there). Quarried rock regrows like any dig edit (the crag heals), and the **original-grade skim rule** (above) means you can only ever shave a rock surface, never hollow it out — so **quarrying is free yet cannot wreck the landscape** (James's requirement). **Licensed deep mining is for the valuable ores only** (coal/iron/jet/alum/polyhalite); a sunk mine also turns up stone, but you never *need* a licence for building stone. (Worldgen already puts `B.STONE` at the surface on `rocky` ground and `B.COBBLE`/`B.GRAVEL` in beck beds; the build ensures those exposures are common enough near play.)
...
m. **Building stone = surface quarrying, not mining (James's flag):** stone/cobble for builds is quarried free at the surface (rocky crags/scars/dale-sides, beck beds), within the 1-block rule; quarried rock regrows. **Controlled by the original-grade skim rule** — the depth gate measures from the *fixed seed grade*, so you can only shave a surface, never gouge or level it: free yet **cannot wreck the landscape** (James's call). The deep **licence is for ores only**, so basic building is never gated behind a mine. Worldgen ensures rocky exposures are common; skim depth + regrowth speed are tuned so stone-gathering is workable, not grindy.
