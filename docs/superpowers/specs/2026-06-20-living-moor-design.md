...
- **The 1-block-deep rule.** At break time, if the target block sits more than one below the natural surface height of that column (`world.gen.height(x,z)`) **and** the player is not within a licensed mine's shaft envelope, the break is refused with a hint ("tha can only dig deep inside a licensed mine — set a mine entrance an' buy a licence"). Surface work (within a block of grade) is always free; this is what makes strip-pits *impossible*, so there are no deep open scars to backfill.
...
**Quarrying vs mining — where building stone comes from (James's flag).** So the 1-block rule never blocks basic building: **building stone is quarried at the surface, free and unlicensed.** The moors expose rock at **crags, scars and dale-sides** (`rocky` ground, where stone *is* the surface) and **beck beds** yield cobble and gravel — all taken within the 1-block-of-local-grade rule (the rock is the grade there). Quarried rock regrows like any dig edit (the crag heals), so quarries don't scar for good. **Licensed deep mining is for the valuable ores only** (coal/iron/jet/alum/polyhalite); a sunk mine also turns up stone, but you never *need* a licence for building stone. (Worldgen already puts `B.STONE` at the surface on `rocky` ground and `B.COBBLE`/`B.GRAVEL` in beck beds; the build ensures those exposures are common enough near play.)
...
- **`src/geography.js` / `src/worldgen.js`** — harvestable **peat** banks at the surface on the boggy tops (the existing `bogginess`), cut at grade (a `harvest` edit that regrows); enough **rocky crag/scar exposures** (`rocky` surface stone) + beck `cobble`/`gravel` that **building stone is quarriable at the surface** without a mine. **`src/economy.js`** — `B.PEAT` gains the **lowest** price (the cheap bulk fuel / trade good).
...
- **Deep-dig gate:** a break more than one below grade → `mayDigDeep(y, grade, mines, heldPick, fixtures)` → allowed only inside a licensed mine's envelope holding the band's pick + fixture; else refused with the reason.
...
m. **Building stone = surface quarrying, not mining (James's flag):** stone/cobble for builds is quarried free at the surface (rocky crags/scars/dale-sides, beck beds), within the 1-block rule (rock is the grade there); quarried rock regrows. The deep **licence is for ores only**, so basic building is never gated behind a mine. Worldgen ensures rocky exposures are common enough near play.
