...
export class Quests {
  ...
  // ---------------- barter ----------------
  tradesFor(villagerName) {
    const n = (villagerName || '').toLowerCase();
    const sIdx = this.standingIndex();
    const all = {
      james: [
        { give: [I.BILBERRIES, 5], get: [I.COOKED_MUTTON, 2], min: 0 },
        { give: [B.WOOL, 4], get: [I.IRON_INGOT, 1], min: 1 },
        { give: [I.JET_GEM, 2], get: [I.I_PICK, 1], min: 3 },
      ],
      glinda: [
        { give: [B.HEATHER, 8], get: [B.WOOL, 2], min: 0 },
        { give: [B.HEATHER, 14], get: [B.LANTERN, 1], min: 2 },
        { give: [I.AMMONITE, 1], get: [I.BILBERRIES, 6], min: 0 },
      ],
      harry: [
        { give: [I.RAW_GROUSE, 2], get: [I.BILBERRIES, 5], min: 0 },
        { give: [I.AMMONITE, 2], get: [I.IRON_INGOT, 1], min: 0 },
        { give: [I.GRYPHAEA, 3], get: [I.COOKED_MUTTON, 3], min: 0 },
      ],
      karen: [
        { give: [B.WOOL, 3], get: [B.WINDOW, 2], min: 1 },
      ],
      cc: [
    ...
    for (const key of Object.keys(all)) {
      if (n.includes(key)) {
        return all[key].map(t => ({ ...t, unlocked: sIdx >= t.min }));
      }
    }
    return [];
  }
  ...
  doTrade(trade) {
    ...
    if (p.countItem(trade.give[0]) < trade.give[1]) {
      this.game.ui.toast(`Tha needs ${trade.give[1]}\u00d7 ${itemName(trade.give[0])} for that swap.`);
      return false;
    }
    p.removeItem(trade.give[0], trade.give[1]);
    const left = p.addItem(trade.get[0], trade.get[1]);
    if (left > 0) this.game.dropAtPlayer(trade.get[0], left);
    this.game.ui.invDirty = true;
