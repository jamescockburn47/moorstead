...
assert.equal(FUTURE_BLOCKS.length, 6);
assert.deepEqual(FUTURE_BLOCKS.map(row => row.id), [200, 201, 202, 203, 204, 205]);
assert.deepEqual(FUTURE_BLOCKS.map(row => row.tile), [120, 121, 122, 123, 124, 125]);
...
assert.equal(hashes.size, 6, 'six blocks have distinct procedural art');
...
// six reserved cells and that all four sides and corners replicate payload edges.
...
assert.equal(patches.length, 6, 'repeat painting reuses the same atlas without work or allocation');
assert.equal(uploads, 1, 'one GPU upload request for six static tiles');
...
assert.deepEqual(Object.keys(BLOCKS), [...originalIds, '200', '201', '202', '203', '204', '205'], 'registration adds only six private IDs');
assert.ok(readFileSync(new URL('../src/freeplay/future-blocks.js', import.meta.url), 'utf8').split('\n').length <= 300);
console.log('PASS free-play future blocks: isolated/idempotent registration, six distinct opaque procedural tiles, gutters, one atlas upload and Uint8 chunk persistence.');
