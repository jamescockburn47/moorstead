...
- All of the above, plus the SP1+SP2 review hardening, is **committed and pushed to `origin/main`** and **deployed to production**. **Git push does NOT auto-deploy** (the GitHub link is metadata-only — verified: a push registered no build): deploy with `npx vercel deploy --prod --yes` from the repo root (Vercel CLI auth is stored locally; `.vercel/` sets the project), or `deploy/ship.ps1`. The current build is **live at www.moorstead.app** (apex `moorstead.app`/`moorcraft.app` 308-redirect there; `/about.html` returns 200). Full deploy flow is in the `moorcraft-evo-stack` memory and the repo `README.md` "Deploying" table (the Vercel client and the four EVO services — brain, relay, dashboard, Merlin — deploy separately). `npm run verify` is now **13 checks**.
