...
| Inn decor layer | `src/innDecor.js` (`InnDecorLayer`) | painted sign (canvas texture) + bracket sign, hearth `Fire()` + chimney smoke plume, night window glow, interior lamp glow, festival mount props (mantel/door lintel), [D4] bragging board above the servery showing the LOCAL player's pub-games record (per-room shared standings deferred with PvP) | main.js:846 | `verify-inn-decor` |
| Pub game engines | `src/games/{merrils,draughts,dominoes,shoveha}.js` + `src/games/ENGINE-CONTRACT.md` | 4 pure, deterministic, JSON-serialisable game engines (`initState`/`legalMoves`/`applyMove`/`winner`/`bestMove`/`summary`); seeded minimax/aim NPC opponents, no THREE/DOM | (by gameTable) | `verify-merrils`, `verify-draughts`, `verify-dominoes`, `verify-shoveha` |
| Pub game table session | `src/gameTable.js` | `tableAt`, opponent-role → minimax level, pure session reducer (`newSession`/`sessionMove`/`sessionNpcReply`/`sessionForfeit`/`settleWager`), board text rendering | main.js interact dispatch | `verify-gametable` |
